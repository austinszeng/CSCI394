    ...
    if (src_stream) {
        ...
        if (curr_char < 0) {
            throw SlpyError { Locn { src_file_name, -1, -1 },
                                 "Immediately encountered error attempting to read the file." };
        }
        start_fresh_token();
    } else {
        throw SlpyError { Locn { src_file_name, -1, -1 },
    ...
    do {
        ...
        case INIT:
            if (curr_char >= '1' && curr_char <= '9') {
                ...
                // If DOS return, ignore it. Assumes \n also ends lines.
                advance_char();
            } else if (curr_char == '#') {
                //
        ...
        case STRG:
            ...
            if (curr_char == '"') {
                ...
                //
                // Hit tab or newline (including DOS one) while processing string literal.
                // ERROR!
                //
                if (curr_char == '\n' || curr_char == '\r') {
